...
class TextCNN(object):
    '''
    A CNN for predicting prices by text
    '''
    def __init__(
        ...
        for i,filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                ...
                conv = tf.nn.conv2d(
                    self.embedded_chars_expanded, #batch_size,sequence_length,embedding_size,channel
                    W,
                    strides=[1,1,1,1],
                    padding='VALID',
                    name='conv'
        ...
        with tf.name_scope('output'):
            W = tf.get_variable(
                ...
                initializer=tf.contrib.layers.xavier_initializer() # W矩阵初始化
            )
        ...
        # CalculateMean mean square error loss
        with tf.name_scope('loss'):
            losses = tf.reduce_mean(tf.square(self.predictions - self.input_y))
